...
cli_show_spanning_tree_detailed_config(const struct ovsrec_mstp_common_instance *cist_row) {
    ...
    for(i=0; i<count; i++) {
        ...
        vty_out(vty, "%-43s:%2"PRId64"%s", "Number of transitions to forwarding state",
...
cli_show_spanning_tree_config(bool detail) {
    ...
    vty_out(vty, "  %34s%"PRId64"  %s%"PRId64"  %s%"PRId64"%s",
    ...
    vty_out(vty, "%s  %-10s %-10s: %-20"PRId64"%s", VTY_NEWLINE, "Bridge ID",
    ...
    for(i=0; i<count; i++) {
        cist_port = (const struct ovsrec_mstp_common_instance_port *)cist_port_nodes[i]->data;
        vty_out(vty, "%-12s %-14s %-10s %-7"PRId64" %-10"PRId64" %s%s",
...
cli_show_mstp_config() {
    ...
    vty_out(vty, "   %-20s : %-15zd%s", "Number of instances",
    ...
    for (i=0; i < bridge_row->n_mstp_instances; i++) {
        ...
        /* Loop for all vlans in one MST instance table */
        vty_out(vty,"%-16"PRId64, bridge_row->key_mstp_instances[i]);
...
mstp_show_common_instance_info(
    ...
    vty_out(vty, "%s", VTY_NEWLINE);
    vty_out(vty, "%-14s %s:%-20s %s:%"PRId64"%s", "Bridge", "Address",
    ...
    vty_out(vty, "%-14s %s:%2"PRId64"  %s:%2"PRId64"  %s:%2"PRId64"  %s:%2"PRId64"%s", "Operational",
            ...
            VTY_NEWLINE);
    vty_out(vty, "%-14s %s:%2"PRId64"  %s:%2"PRId64"  %s:%2"PRId64"  %s:%2"PRId64"%s", "Configured",
            "Hello time(in seconds)",
    ...
    vty_out(vty, "%-14s Address:%-20s Priority:%"PRId64"%s", "Root",
            root_mac,
            (cist_row->root_priority)?(*cist_row->root_priority):(DEF_BRIDGE_PRIORITY * MSTP_BRIDGE_PRIORITY_MULTIPLIER),
            VTY_NEWLINE);

    vty_out(vty, "%19s:%s, Cost:%"PRId64", Rem Hops:%"PRId64"%s", "Port",
    ...
    for(j=0; j<count; j++) {
        ...
        vty_out(vty, "%-14s %-14s %-10s %-10"PRId64" %-10"PRId64" %s%s",
...
mstp_show_common_instance_port_info(
    ...
    vty_out(vty, "%-10s %s %"PRId64" sec, %s:%"PRId64", %s:%"PRId64"%s", "Timers:",
                 "Message expires in",
                 (cist_row->hello_expiry_time)?*cist_row->hello_expiry_time:0,
                 "Forward delay expiry",
                 (cist_row->forward_delay_expiry_time)?*cist_row->forward_delay_expiry_time:0,
...
mstp_show_instance_info(int64_t inst_id, const struct ovsrec_mstp_instance *mstp_row) {
    ...
    vty_out(vty, "%s%s%"PRId64"%s%s  ", VTY_NEWLINE, "#### MST",
    ...
    for (j=0; j < mstp_row->n_mstp_instance_ports; j++) {
        ...
        if (mstp_port->port) {
            vty_out(vty, "%-14s %-14s %-10s %-7"PRId64" %-10"PRId64" %s%s",
...
cli_show_mst_interface(int inst_id, const char *if_name, bool detail) {
    ...
    if(detail) {
        ...
        vty_out(vty, "%s %s%s", "-------------- --------------",
                "---------- ---------- ---------- ----------", VTY_NEWLINE);
        if(inst_id != MSTP_CISTID) {
            vty_out(vty, "%-14d %-14s %-10s %-10"PRId64" %-11"PRId64"", inst_id,
    ...
    if (bridge_row) {
        ...
        for (i=0; i < bridge_row->n_mstp_instances; i++) {
            ...
            for (j=0; j<mstp_row->n_vlans; j++) {
                sprintf(str, "%"PRId64"", mstp_row->vlans[j]->id);
            ...
            /* Loop for all vlans in one MST instance table */
            for (j=0; j<mstp_row->n_vlans; j++) {
                vty_out(vty, "spanning-tree instance %"PRId64" vlan %"PRId64"%s",
            ...
            if (mstp_row->priority &&
                ...
                vty_out(vty, "spanning-tree instance %"PRId64" priority %"PRId64"%s",
                bridge_row->key_mstp_instances[i],
...
cli_show_mstp_intf_config() {
    ...
    for (i=0; i < cist_row->n_mstp_common_instance_ports; i++) {
        ...
        if (cist_port_row->admin_path_cost &&
            ...
            vty_out(vty, "%4s%s %"PRId64"%s", "", "spanning-tree cost",
        ...
        if (cist_port_row->port_priority &&
            ...
            vty_out(vty, "%4s%s %"PRId64"%s", "", "spanning-tree port-priority",
        ...
        for (j=0; j < bridge_row->n_mstp_instances; j++) {
            ...
            for (k=0; k<mstp_row->n_mstp_instance_ports; k++) {
                ...
                if (mstp_port_row->port_priority &&
                    ...
                    vty_out(vty, "%4s%s %"PRId64" %s %"PRId64"%s", "",
...
mstp_cli_remove_inst_vlan_map(const int64_t instid, const char *vlanid, struct ovsdb_idl_txn *txn) {
    ...
    if(vlanid)
    {
        ...
        if (!vlan_row) {
            vty_out(vty, "Error : Vlan ID-%"PRId64" is not created, aborting the VLAN's ",vlan_id);
            cli_do_config_abort(txn);
            return CMD_WARNING;
        }
    }
...
DEFUN(cli_mstp_port_type,
    ...
    mstp_cli_set_cist_port_table((char*)((uintptr_t)vty->index), MSTP_ADMIN_EDGE, value);
    return CMD_SUCCESS;
}
...
DEFUN(cli_no_mstp_port_type_admin,
    ...
    mstp_cli_set_cist_port_table((char*)((uintptr_t)vty->index), MSTP_ADMIN_EDGE, DEF_ADMIN_EDGE);
    return CMD_SUCCESS;
}
...
DEFUN(cli_mstp_bpdu,
    ...
    mstp_cli_set_cist_port_table((char*)((uintptr_t)vty->index), argv[0], true);
    return CMD_SUCCESS;
}
...
DEFUN(cli_mstp_bpdu_enable,
    ...
    mstp_cli_set_cist_port_table((char*)((uintptr_t)vty->index), argv[0], value);
    return CMD_SUCCESS;
}
...
DEFUN(cli_no_mstp_bpdu_enable,
    ...
    mstp_cli_set_cist_port_table((char*)((uintptr_t)vty->index), argv[0], DEF_BPDU_STATUS);
    return CMD_SUCCESS;
}
...
DEFUN(cli_mstp_port_priority,
    ...
    mstp_cli_set_string_cist_port_table((char*)((uintptr_t)vty->index), MSTP_PORT_PRIORITY, atoi(argv[0]));
    return CMD_SUCCESS;
}
...
DEFUN(cli_no_mstp_port_priority,
    ...
    mstp_cli_set_string_cist_port_table((char*)((uintptr_t)vty->index), MSTP_PORT_PRIORITY, DEF_MSTP_PORT_PRIORITY);
    return CMD_SUCCESS;
}
...
DEFUN(cli_mstp_cost,
    ...
    mstp_cli_set_string_cist_port_table((char*)((uintptr_t)vty->index), MSTP_PORT_COST, atoi(argv[0]));
    return CMD_SUCCESS;
}
...
DEFUN(cli_no_mstp_cost,
    ...
    mstp_cli_set_string_cist_port_table((char*)((uintptr_t)vty->index), MSTP_PORT_COST, DEF_MSTP_COST);
    return CMD_SUCCESS;
}
...
DEFUN(cli_mstp_inst_cost,
    ...
    mstp_cli_set_mst_inst((char*)((uintptr_t)vty->index), MSTP_PORT_COST, atoi(argv[0]), atoi(argv[1]));
    return CMD_SUCCESS;
}
...
DEFUN(cli_no_mstp_inst_cost,
    ...
    mstp_cli_set_mst_inst((char*)((uintptr_t)vty->index), MSTP_PORT_COST, atoi(argv[0]),
                                                  DEF_MSTP_COST);
    return CMD_SUCCESS;
}
...
DEFUN(cli_mstp_inst_port_priority,
    ...
    mstp_cli_set_mst_inst((char*)((uintptr_t)vty->index), MSTP_PORT_PRIORITY, atoi(argv[0]),
                                                      atoi(argv[1]));
    return CMD_SUCCESS;
}
...
DEFUN_HIDDEN(cli_mstp_inst_port_state,
    ...
    mstp_cli_set_mist_port_state((char*)((uintptr_t)vty->index), atoi(argv[0]), argv[1]);
    return CMD_SUCCESS;
}
...
DEFUN_HIDDEN(cli_mstp_port_state,
    ...
    mstp_cli_set_cist_port_state((char*)((uintptr_t)vty->index), argv[0]);
    return CMD_SUCCESS;
}
#endif
